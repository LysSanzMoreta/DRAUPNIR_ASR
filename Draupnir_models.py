...
class DRAUPNIRModel_classic_Transformer(DRAUPNIRModelClass):
    "Blosum weighted average embedding. Training on leaves, testing on internal nodes, no batching"
    def __init__(self,ModelLoad):
        DRAUPNIRModelClass.__init__(self,ModelLoad)
        self.rnn_input_size = self.z_dim + self.aa_prob
        ...
        self.num_layers = 3
        self.decoder = TransformerDecoder_Tiling(self.max_seq_len, self.aa_prob, self.gru_hidden_dim, self.z_dim, self.rnn_input_size,self.kappa_addition,self.num_layers,self.pretrained_params)
        self.embed = EmbedComplex(self.aa_prob,self.embedding_dim, self.pretrained_params)
    def model(self, family_data, patristic_matrix_sorted,cladistic_matrix,clade_blosum = None):
        aminoacid_sequences = family_data[:, 2:, 0]
        batch_nodes = family_data[:, 0, 1]
    ...
    def sample(self, map_estimates, n_samples, family_data_test, patristic_matrix,cladistic_matrix,use_argmax=False,use_test=True,use_test2=False):
        ...
        with pyro.plate("plate_len",self.max_seq_len, dim=-1), pyro.plate("plate_seq",n_nodes,dim=-2,subsample_size=n_nodes):
            logits = self.decoder.forward(
                sequences = None,
                latent=latent_space_)
            if use_argmax: #Highlight; We use it for marginal and variational
                #Pick the sequence with the highest likelihood, now n_samples, n_samples = 1
                aa_sequences = torch.argmax(logits,dim=2).unsqueeze(0) #I add one dimension at the beginning to resemble 1 sample and not have to change all the plotting code
            else: #Highlight: We only use it for MAP
                aa_sequences = dist.Categorical(logits=logits).sample([n_samples])
...
class DRAUPNIRModel_classic_no_blosum_SRU(DRAUPNIRModelClass):
    "OU process alone, no blosum embeddings"
    def __init__(self,ModelLoad):
        DRAUPNIRModelClass.__init__(self,ModelLoad)
        self.rnn_input_size = self.z_dim
        self.num_layers = 3
        self.plate_size = [self.plate_size if self.plate_size is not None else self.n_leaves][0] #TODO: Check that always holds
        self.decoder = SRUDecoder_Tiling(self.max_seq_len, self.aa_prob, self.gru_hidden_dim, self.z_dim,
                                         self.rnn_input_size, self.kappa_addition, self.num_layers,
    ...
    def model(self, family_data, patristic_matrix_sorted,cladistic_matrix,clade_blosum = None):
        ...
        # Highlight: MAP the latent space to logits using the Decoder from a Seq2seq model with/without attention
        latent_space = latent_space.repeat(1,self.max_seq_len).reshape(latent_space.shape[0],self.max_seq_len,self.z_dim) #[n_nodes,max_seq,z_dim]
        decoder_hidden = self.h_0_MODEL.expand(self.decoder.num_layers * 2, self.plate_size,
                                               self.gru_hidden_dim).contiguous()  # bidirectional
...
class DRAUPNIRModel_classic_no_blosum_Transformer(DRAUPNIRModelClass):
    "Blosum weighted average embedding. Training on leaves, testing on internal nodes, no batching"
    def __init__(self,ModelLoad):
        DRAUPNIRModelClass.__init__(self,ModelLoad)
        self.rnn_input_size = self.z_dim
...
class DRAUPNIRModel_classic_SRU(DRAUPNIRModelClass):
    "Blosum weighted average embedding. Training on leaves, testing on internal nodes, no batching"
    def __init__(self,ModelLoad):
        DRAUPNIRModelClass.__init__(self,ModelLoad)
        self.rnn_input_size = self.z_dim + self.aa_prob
        # self.decoder_attention = RNNAttentionDecoder(self.n_leaves, self.max_seq_len, self.aa_prob, self.gru_hidden_dim,self.rnn_input_size,
        #                                              self.embedding_dim, self.z_dim, self.kappa_addition)
        #Highlight: Increased layers from 1 to 3
        self.plate_size = [self.plate_size if self.plate_size is not None else self.n_leaves][0]
        self.num_layers = 3
        self.decoder = SRUDecoder_Tiling(self.max_seq_len, self.aa_prob, self.gru_hidden_dim, self.z_dim, self.rnn_input_size,self.kappa_addition,self.num_layers,self.pretrained_params)
        self.embed = EmbedComplex(self.aa_prob,self.embedding_dim, self.pretrained_params)
    def model(self, family_data, patristic_matrix_sorted,cladistic_matrix,clade_blosum = None):
        aminoacid_sequences = family_data[:, 2:, 0]
        batch_nodes = family_data[:, 0, 1]
...
class DRAUPNIRModel_batching(DRAUPNIRModelClass):
    ...
    def sample(self, map_estimates, n_samples, family_data_test, patristic_matrix,cladistic_matrix,use_argmax=False,use_test=True,use_test2=False):
        ...
        print(map_estimates["latent_z"].shape)
        print("Not implemented. Does it make sense to divide the leaves latent space? How to concatenate it again?")
        exit()
        if use_test or use_test2:
            assert patristic_matrix[1:,1:].shape == (self.n_all,self.n_all)
            latent_space = self.conditional_sampling(map_estimates,patristic_matrix)
...
class DRAUPNIRModel_anglespredictions_SRU(DRAUPNIRModelClass):
    "Leaves training and testing.Predicting both ANGLES and AA sequence. Working on full or partial leaves space"
    def __init__(self,ModelLoad):
        DRAUPNIRModelClass.__init__(self,ModelLoad)
        self.rnn_input_size = self.z_dim + self.aa_prob
        #Highlight: Increased num_layers from 1 to 6 for single sru
        #self.decoder = RNNDecoder_Angles_Single_SRU(self.max_seq_len, self.aa_prob, self.gru_hidden_dim, self.z_dim, self.rnn_input_size,self.kappa_addition,self.num_layers,self.pretrained_params)
        self.decoder = RNNDecoder_Angles_Double_SRU(self.max_seq_len, self.aa_prob, self.gru_hidden_dim, self.z_dim, self.rnn_input_size,self.kappa_addition,self.num_layers,self.pretrained_params)
    ...
    def model(self, family_data, patristic_matrix_sorted,cladistic_matrix,clade_blosum):
        ...
        with pyro.plate("plate_len", aminoacid_sequences.shape[1], dim=-1), pyro.plate("plate_seq",aminoacid_sequences.shape[0],dim=-2):
            ...
            kappas = kappas[train_indexes]
            pyro.sample("aa_sequences", dist.Categorical(logits=logits), obs=aminoacid_sequences) #aa_seq = [n_nodes,max_seq_len]
            pyro.sample("phi",dist.VonMises(loc = means[:,:,0],concentration = kappas[:,:,0]), obs=angles[:,:,0])
            pyro.sample("psi",dist.VonMises(loc = means[:,:,1],concentration = kappas[:,:,1]), obs=angles[:,:,1])
...
class DRAUPNIRModel_sparseGP(DRAUPNIRModelClass):
    "Sparse GP prior on latent space. Repeat technique + aa freqs TODO: Finish"
    def __init__(self,ModelLoad):
        DRAUPNIRModelClass.__init__(self,ModelLoad)
        self.rnn_input_size = self.z_dim + self.aa_prob
        self.chunk_size = DraupnirUtils.Define_batch_size(self.n_nodes)  # divide into equal sizes(integers, not floats)---> the size of the chunk is in the first tuple
        #self.decoder_attention = RNNAttentionDecoder(self.n_nodes, self.max_seq_len, self.aa_prob, self.gru_hidden_dim,self.rnn_input_size,self.embedding_dim, self.z_dim, self.kappa_addition)
        self.decoder = RNNDecoder_Tiling(self.max_seq_len, self.aa_prob, self.gru_hidden_dim, self.z_dim, self.rnn_input_size,
                                  self.kappa_addition,self.num_layers,self.pretrained_params)
    def model(self, family_data, patristic_matrix_sorted,cladistic_matrix,clade_blosum):
        aminoacid_sequences = family_data[:, 2:, 0]
        angles = family_data[:, 2:, 1:3]
        # Highlight: Register GRU module
        ...
        sigma_n = pyro.sample("sigma_n", dist.HalfNormal(alpha[1]).expand_by([self.z_dim, ]).to_event(1))  # Gaussian noise
        lambd = pyro.sample("lambd", dist.HalfNormal(alpha[2]).expand_by([self.z_dim, ]).to_event(1))  # characteristic length-scale
        # Highlight: Sample from a MultivariateNormal for every dimension in the latent space, use an sparse covariance matrix
        patristic_mean = torch.mean(patristic_matrix_sorted[1:,1:])
        patristic_std = torch.std(patristic_matrix_sorted[1:,1:])
        #n_inducing_points =
        inducing_set = pyro.sample("inducing_set",dist.Normal(patristic_mean,patristic_std)) #number of leaves to use as inducing points

        patristic_matrix = patristic_matrix_sorted[1:, 1:]


        latent_space = pyro.sample('latent_z',VSGP(OUKernel_Fast_Sparse(sigma_f, sigma_n, lambd),inducing_set,dist.MultivariateNormal, input_data=patristic_matrix))

        #latent_space = pyro.sample('latent_z', dist.MultivariateNormal(OU_mean[None, :], OU_covariance).to_event(1))
        latent_space = latent_space.T
        # Highlight: MAP the latent space to logits using the Decoder from a Seq2seq model with/without attention
        decoder_hidden = self.h_0_MODEL.expand(self.decoder.num_layers * 2, latent_space.shape[0],
                                               self.gru_hidden_dim).contiguous()  # Not bidirectional

        latent_space = latent_space.repeat(1,self.max_seq_len).reshape(family_data.shape[0],self.max_seq_len,self.z_dim)
        # Highlight: MAP the latent space to logits using the Decoder from a Seq2seq model with/without attention

        with pyro.plate("plate_len", aminoacid_sequences.shape[1], dim=-1):
            with pyro.plate("plate_seq", aminoacid_sequences.shape[0], dim=-2):
                if self.use_attention:
                    logits = self.decoder_attention.forward(
                        input=latent_space,
                        hidden=decoder_hidden)
                else:
                    logits = self.decoder.forward(
    ...
    def sample(self, map_estimates, n_samples, family_data_test, patristic_matrix_test,use_test=True,use_test2=False,use_argmax=True):

        latent_space = torch.zeros((family_data_test.shape[0], self.z_dim))
        sigma_f = map_estimates["sigma_f"]  # 30 dim vectors, so should be valid for everything
        sigma_n = map_estimates["sigma_n"]
        lambd = map_estimates["lambd"]
        inducing_set = map_estimates["inducing_set"]

        latent_space = VSGP(OUKernel_Fast_Sparse(sigma_f, sigma_n, lambd),inducing_set ,dist.MultivariateNormal,input_data=patristic_matrix_test[1:, 1:]).sample()
        latent_space = latent_space.T

        decoder_hidden = self.h_0_MODEL.expand(self.decoder.num_layers * 2, latent_space.shape[0],
                                               self.gru_hidden_dim).contiguous()  # Not bidirectional
        latent_space =  latent_space.repeat(1,self.max_seq_len).reshape(family_data_test.shape[0],self.max_seq_len,self.z_dim)
        aa_freqs = self.aa_frequencies
        epsilon = map_estimates["epsilon"]
        aa_freqs = torch.where(aa_freqs == 0., aa_freqs, aa_freqs - epsilon)
        aa_freqs = torch.where(aa_freqs != 0., aa_freqs, epsilon / (self.aa_prob - 1))
        aa_freqs = aa_freqs.repeat(family_data_test.shape[0],1).reshape(family_data_test.shape[0],self.max_seq_len,self.aa_prob)

        latent_space = torch.cat((latent_space, aa_freqs), dim=2)
        with pyro.plate("plate_len", self.max_seq_len, dim=-1), pyro.plate("plate_seq", n_nodes, dim=-2):
            logits = self.decoder.forward(
                input=latent_space_extended,
                hidden=decoder_hidden)
            ...
            # batch_logits = logits[batch_indexes]
            if use_argmax:  # Pick the sequence with the highest likelihood, now n_samples, n_samples = 1
                aa_sequences = torch.argmax(logits, dim=2).unsqueeze(
                    0)  # I add one dimension at the beginning to resemble 1 sample and not have to change all the plotting code
            else:
                aa_sequences = dist.Categorical(logits=logits).sample([n_samples])
            # return aa_sequences,latent_space, logits, None, None
        sampling_out = SamplingOutput(aa_sequences=aa_sequences.detach(),
