...
def datasets_pretreatment(train_load,test_load,additional_load,build_config,device,name,simulation_folder,root_sequence_name,dataset_number,settings_config):
    ...
    if name.startswith("simulation"):
    ...
    elif name in ["ANC_A1_subtree","ANC_A2_subtree","ANC_AS_subtree","ANC_S1_subtree"]:
        ...
        if not build_config.no_testing:
            print("Leave testing: Only sorting the train/test dataframes")
...
def convert_to_integers(sample_out):
    sample_out = SamplingOutput(
        aa_sequences=DraupnirUtils.ConvertToIntegers(sample_out.aa_sequences.cpu(), build_config.aa_prob, axis=3),
...
def select_quide(Draupnir,model_load,choice):
    ...
    if choice in guide_types.keys(): return guide_types[choice]
        ...
        return guide
def histogram_density(logits,folder):
...
def draupnir_sample(train_load, test_load, additional_load, additional_info, build_config, settings_config, n_samples,
    ...
    with torch.no_grad():
        for name, parameter in guide.named_parameters():
            #if name in pretrained_params_dict_guide.keys():
    ...
    if select_guide == "variational":
        ...
        sample_out_test_argmax2 = Draupnir.sample(map_estimates,
                                                  ...
                                                  use_test2=True,
                                                  use_test=False)
        # sample_out_test_argmax2 = SamplingOutput(aa_sequences=torch.mode(sample_out_test2.aa_sequences,dim=0)[0].unsqueeze(0),
        #                                         latent_space=sample_out_test2.latent_space,
        #                                         logits=sample_out_test2.logits,
        #                                         phis=None,
        #                                         psis=None,
        #                                         mean_phi=None,
        #                                         mean_psi=None,
    ...
    visualize_latent_space(sample_out_train_argmax.latent_space, sample_out_test_argmax2.latent_space, patristic_matrix_train,
                           patristic_matrix_test, additional_load, additional_load, folder="/Test2_Plots")
    ...
    print("##########################################################################################################")
def draupnir_sample_from_logits(train_load, test_load, additional_load, additional_info, build_config, settings_config, n_samples,
                   graph_coo=None, clades_dict=None):
    if not additional_load.correspondence_dict:
        correspondence_dict = dict(
            zip(list(range(len(additional_load.tree_levelorder_names))), additional_load.tree_levelorder_names))
    ...
    print("¡¡¡Loading logits from previously trained results!!!")
    def sample_logits(logits, latent_space, use_argmax):  # TODO: Generalize for angles too
        if use_argmax:
            ...
            aa_sequences = pyro.distributions.Categorical(logits=logits).sample([args.n_samples])
    ...
    load_folder = args.load_pretrained_path
    ...
    if not os.path.exists("{}/Train_Plots/train_info_dict.torch".format(load_folder)):
        raise ValueError("This folder does not have train_info_dict.torch")
    ...
    sample_out_train = DraupnirLoadUtils.load_dict_to_namedtuple(train_dict)
    sample_out_train = sample_logits(sample_out_train.logits,sample_out_train.latent_space,use_argmax=False)
    sample_out_test = DraupnirLoadUtils.load_dict_to_namedtuple(test_dict)
    sample_out_test = sample_logits(sample_out_test.logits, sample_out_test.latent_space, use_argmax=False)
    sample_out_test_argmax = DraupnirLoadUtils.load_dict_to_namedtuple(test_argmax_dict)
    sample_out_test_argmax = sample_logits(sample_out_test_argmax.logits,sample_out_test_argmax.latent_space,use_argmax=True)
    try:  # some old predictions do not have train_argmax folder
        ...
        sample_out_train_argmax = DraupnirLoadUtils.load_dict_to_namedtuple(train_argmax_dict)
        sample_out_train_argmax = sample_logits(sample_out_train_argmax.logits, sample_out_train_argmax.latent_space,use_argmax=True)
    except:
        print("Could not load train_argmax folders, creating dummy dataset")
        sample_out_train_argmax = SamplingOutput(
            aa_sequences=torch.zeros(dataset_train[:, 0, 1].shape),  # This is completely off
            latent_space=torch.ones((dataset_train.shape[0], int(config["z_dim"]))),
            logits=torch.rand((dataset_train.shape[0], max_seq_len, build_config.aa_prob)),
            phis=None,
            psis=None,
            mean_phi=None,
            mean_psi=None,
    ...
    try:  # some old predictions do not have test2 folders
        ...
        sample_out_test_argmax2 = DraupnirLoadUtils.load_dict_to_namedtuple(test_argmax_dict2)
        sample_out_test_argmax2 = sample_logits(sample_out_test_argmax2.logits, sample_out_test_argmax2.latent_space,
                                               use_argmax=True)

        sample_out_test2 = DraupnirLoadUtils.load_dict_to_namedtuple(test_dict2)
        sample_out_test2 = sample_logits(sample_out_test2.logits, sample_out_test2.latent_space, use_argmax=False)

        test_entropies2 = test_dict2["entropies"]
    except:
        print("Could not load test2 folders!!!---> the results are not from a trained run")
        sample_out_test2 = SamplingOutput(
            aa_sequences=torch.zeros((n_samples, patristic_matrix_test.shape[0] - 1, max_seq_len)),
            latent_space=torch.ones((patristic_matrix_test.shape[0] - 1, int(config["z_dim"]))),
            logits=torch.rand((patristic_matrix_test.shape[0] - 1, max_seq_len, build_config.aa_prob)),
            phis=None,
            psis=None,
            mean_phi=None,
            mean_psi=None,
            kappa_phi=None,
            kappa_psi=None)
        sample_out_test_argmax2 = SamplingOutput(
            aa_sequences=torch.zeros((1, patristic_matrix_test.shape[0] - 1, max_seq_len)),
            latent_space=torch.ones((patristic_matrix_test.shape[0] - 1, int(config["z_dim"]))),
            logits=torch.rand((patristic_matrix_test.shape[0] - 1,max_seq_len, build_config.aa_prob)),
            phis=None,
            psis=None,
    ...
    save_samples(dataset_test, patristic_matrix_test, sample_out_test, test_entropies, correspondence_dict,
                 "{}/test_info_dict.torch".format(RESULTS_DIR + "/Test_Plots"))
    save_samples(dataset_test, patristic_matrix_test, sample_out_test_argmax, test_entropies, correspondence_dict,
                 "{}/test_argmax_info_dict.torch".format(RESULTS_DIR + "/Test_argmax_Plots"))
    save_samples(dataset_test, patristic_matrix_test, sample_out_test2, test_entropies2, correspondence_dict,
                 "{}/test_info_dict2.torch".format(RESULTS_DIR + "/Test2_Plots"))
    save_samples(dataset_test, patristic_matrix_test, sample_out_test_argmax2, test_entropies2, correspondence_dict,
                 "{}/test2_argmax_info_dict.torch".format(RESULTS_DIR + "/Test2_argmax_Plots"))
    save_samples(dataset_train, patristic_matrix_train, sample_out_train, train_entropies, correspondence_dict,
                 "{}/train_info_dict.torch".format(RESULTS_DIR + "/Train_Plots"))
    save_samples(dataset_train, patristic_matrix_train, sample_out_train_argmax, train_entropies,
                 correspondence_dict, "{}/train_argmax_info_dict.torch".format(RESULTS_DIR + "/Train_argmax_Plots"))

    histogram_density(sample_out_test.logits, folder="Test_Plots")
    histogram_density(sample_out_test2.logits, folder="Test2_Plots")
    histogram_density(sample_out_test_argmax.logits, folder="Test_argmax_Plots")
    histogram_density(sample_out_test_argmax2.logits, folder="Test2_argmax_Plots")


    visualize_latent_space(sample_out_train.latent_space, sample_out_test.latent_space, patristic_matrix_train,
                           patristic_matrix_test, additional_load, additional_info, folder="")
    visualize_latent_space(sample_out_train.latent_space, sample_out_test_argmax2.latent_space, patristic_matrix_train,
                           patristic_matrix_test, additional_load, additional_load, folder="/Test2_Plots")
...
def draupnir_train(train_load,test_load,additional_load,additional_info,build_config,settings_config,n_samples,graph_coo=None,clades_dict=None):
    ...
    if args.use_scheduler: #TODO: Scheduler not working....
    ...
    plot_probs = False
    if plot_probs:
        #Highlight: Plot the logits/probability density
        histogram_density(sample_out_test.logits,folder="Test_Plots")
        histogram_density(sample_out_test2.logits,folder="Test2_Plots")
        histogram_density(sample_out_test_argmax.logits,folder="Test_argmax_Plots")
        histogram_density(sample_out_test_argmax2.logits,folder="Test2_argmax_Plots")
    ...
    if args.infer_angles: #TODO: not correct anymore
        preparing_plots(sample_out_train, dataset_train, dataset_train, train_entropies,
                        RESULTS_DIR + "/Train_Plots", additional_load, additional_info, n_samples,
                        dataset_train[:, 0, 1], replacement_plots=False, plot_test=False,plot_angles=True,no_testing=True)
        preparing_plots(sample_out_test, dataset_test, dataset_train, test_entropies,
                        RESULTS_DIR + "/Test_Plots", additional_load, additional_info, n_samples,
                        patristic_matrix_test[1:, 0], replacement_plots=False, overplapping_hist=False,plot_angles=True,no_testing=build_config.no_testing)
        preparing_plots(sample_out_test2, dataset_test, dataset_train, test_entropies2,
                        RESULTS_DIR + "/Test2_Plots", additional_load, additional_info, n_samples,
                        patristic_matrix_test[1:, 0], replacement_plots=False, overplapping_hist=False,plot_angles=True,no_testing=build_config.no_testing)
    ...
    #Highlight: Plot samples
    print("train")
    preparing_plots(sample_out_train, dataset_train, dataset_train, train_entropies,RESULTS_DIR + "/Train_Plots",additional_load,additional_info,n_samples,dataset_train[:,0,1],replacement_plots=False,plot_test=False,no_testing=True,overplapping_hist=False)
    print("test")
    preparing_plots(sample_out_test,dataset_test,dataset_train,test_entropies,RESULTS_DIR + "/Test_Plots",additional_load,additional_info,n_samples,patristic_matrix_test[1:,0],replacement_plots=False,overplapping_hist=False,no_testing=build_config.no_testing)
    print("test2")
    preparing_plots(sample_out_test2,dataset_test,dataset_train,test_entropies2,RESULTS_DIR + "/Test2_Plots",additional_load,additional_info,n_samples,patristic_matrix_test[1:,0],replacement_plots=False,overplapping_hist=False,no_testing=build_config.no_testing)
    ...
    #Highlight: Plot most likely sequence
    print("train argmax")
    preparing_plots(sample_out_train_argmax, dataset_train, dataset_train, train_entropies,RESULTS_DIR + "/Train_argmax_Plots",additional_load,additional_info,n_samples,dataset_train[:,0,1],replacement_plots=False,plot_test=False,no_testing=True,overplapping_hist=False)
    print("test argmax")
    preparing_plots(sample_out_test_argmax, dataset_test, dataset_train, test_entropies,RESULTS_DIR + "/Test_argmax_Plots",additional_load,additional_info,n_samples, patristic_matrix_test[1:,0],replacement_plots=False, overplapping_hist=False,no_testing=build_config.no_testing)
    print("test_argmax 2")
    preparing_plots(sample_out_test_argmax2, dataset_test, dataset_train, test_entropies2,RESULTS_DIR + "/Test2_argmax_Plots",additional_load,additional_info,n_samples, patristic_matrix_test[1:,0],replacement_plots=False, overplapping_hist=False,no_testing=build_config.no_testing)

    stop_plots = time.time()
    print('Final plots timing: {}'.format(str(datetime.timedelta(seconds=stop_plots - start_plots))))
    print("##########################################################################################################")
def preparing_plots(samples_out,dataset_test,dataset_train,entropies,results_dir,additional_load,additional_info,n_samples,test_ordered_nodes,replacement_plots=False,overplapping_hist=False,plot_test=True,plot_angles=False,no_testing=False):
    ...
    if plot_test and no_testing:
        ...
        DraupnirPlots.save_ancestors_predictions(name, dataset_test, samples_out.aa_sequences, n_samples, results_dir,
...
def main(config):
    ...
    if args.generate_samples:
        ...
        if args.use_trained_logits:
            draupnir_sample_from_logits(train_load,test_load,additional_load,additional_info,build_config,settings_config,args.n_samples,graph_coo,clades_dict)

        else:
            draupnir_sample(train_load,test_load,additional_load,additional_info,build_config,settings_config,args.n_samples,graph_coo,clades_dict)
