...
#Info of pytests fixtures https://betterprogramming.pub/understand-5-scopes-of-pytest-fixtures-1b607b5c19ed
#                         https://medium.com/ideas-at-igenius/fixtures-and-parameters-testing-code-with-pytest-d8603abb390a
#info on run pytest in pycharm. https://stackoverflow.com/questions/6397063/how-do-i-configure-pycharm-to-run-py-test-tests
#possible badge: https://stackoverflow.com/questions/29295965/python-coverage-badges-how-to-get-
...
@pytest.fixture(scope="class",params=['simulations_blactamase_1', 'aminopeptidase', 'benchmark_randall_original_naming'])
def prepare_dataset(request):
    ...
    args = Args(name=request.param,
                ...
                embedding_dim=50,
                use_cuda=True,
                use_scheduler=False,activate_elbo_convergence=False,
                activate_entropy_convergence=False,
                test_frequency=100,
                config_dict="",
                parameter_search=False)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    #global build_config,settings_config, root_sequence_name
...
@pytest.mark.usefixtures("prepare_dataset")
class DataLoadTest(unittest.TestCase):
    # @pytest.fixture(autouse=True)
    # def _prepare_dataset(self, prepare_dataset):
    #     self._response = prepare_dataset
    def test_load_build_config(self,prepare_dataset):
        print("Done")
        # build_config, settings_config, root_sequence_name = self._response
        # isbuildconfig = [True if build_config.__module__ =="build_config" else False][0]
        # self.assertEqual(isbuildconfig,True)

    # def test_load_settings_config(self,name):
    #     build_config, settings_config, root_sequence_name = prepare_dataset(name)
    #     issettingsconfig = [True if settings_config.__module__ == "settings_config" else False][0]
    #     self.assertEqual(issettingsconfig, True)
