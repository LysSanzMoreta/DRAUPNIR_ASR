...
class TestModel():
    ...
    def generate_samples(self,train_patristic,full_patristic,n_samples = 10):
        ...
        assert self.method in ["0","1","3","5"], f"{self.method} not found"
        ...
        map_estimates_collection = defaultdict(list)
        latent_samples = []
        covariance_samples = []
        for i in range(n_samples):
            ...
            for key,val in map_estimates["ou_params"].items():
                if val.ndim == 0:
                    map_estimates_collection[key].append(torch.Tensor([val])[None, :])
                else:
                    map_estimates_collection[key].append(val[None,:])
        ...
        for key,vals in map_estimates_collection.items():
            map_estimates_collection[key] = torch.concat(vals,axis=0)
...
#the clades have the tree traversal name , how to associate that with the matrix (probably in drauonir load data or utils)

row_names = np.arange(n_seqs)
...
#TODO: perhaps try to find a dataset that generates a dummy binary graph, or create a tree from this one
latent_samples,covariance_samples, map_estimates_collection = TestModel(init_params,test_patristic,tree_height=1).generate_samples(train_patristic,full_patristic,n_samples=5)
