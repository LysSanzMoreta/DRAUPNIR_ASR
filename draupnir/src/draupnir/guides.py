...
class DRAUPNIRGUIDES(EasyGuide):
    ...
    def __init__(self,draupnir_model,ModelLoad, Draupnir):
        ...
        if self.draupnir.args.covariance_prior == "og":
            ...
            self.log_lambd_loc = PyroParam(torch.log(torch.tensor([10.])))
            self.log_lambd_scale = PyroParam(torch.tensor([0.1]),constraint=constraints.positive)
    ...
    def guide(self, datasets, patristic_matrix, patristic_matrix_eval, data_blosum, batch_blosum=None,map_estimates=None):
        """
        :param patristic_matrix: matrix of patristic distances (branch lengths) between the nodes in the tree
        :param patristic_matrix_eval: matrix of cladistic distances between the nodes in the tree
...
class DRAUPNIRGuides_classic(DRAUPNIRGUIDES):
    ...
    def guide_not_batch_experiment5(self, datasets, patristic_matrix_sorted, patristic_matrix_eval, data_blosum,batch_blosum=None, map_estimates=None):
        """
        ...
        log_lambd = pyro.sample("log_lambd", dist.Normal(self.log_lambd_loc, self.log_lambd_scale).expand_by([1]))  # characteristic length-scale
        with pyro.poutine.scale(scale=map_estimates["kl_annealing_factor"] if map_estimates is not None else torch.Tensor([1.])):
    ...
    def guide(self, datasets, patristic_matrix, patristic_matrix_eval, data_blosum, batch_blosum=None,map_estimates=None):
        ...
        if self.batch_size == None or self.batch_size > 1:
            if self.batch_by_clade:
                ...
                return self.guide_batch(datasets, patristic_matrix, patristic_matrix_eval, data_blosum,batch_blosum=None,map_estimates=map_estimates)
        else:
            return self.guide_noplating(datasets, patristic_matrix, patristic_matrix_eval, data_blosum,batch_blosum=None,map_estimates=map_estimates)

    def guide_noplating(self,datasets, patristic_matrix_sorted,patristic_matrix_eval,data_blosum,batch_blosum=None,map_estimates=None):
        """
        :param tensor data_blosum here is the ENTIRE data encoded in blosum vector form instead of integers ---> EQUAL to self.dataset_train_blosum
        """
        #aminoacid_sequences = datasets["blosum"][:, 2:, 0]
